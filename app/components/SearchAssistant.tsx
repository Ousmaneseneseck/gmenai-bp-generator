...
// ⚠️ On accepte les props (même si on ne les utilise pas encore)
...
export default function SearchAssistant({ sector = "Technologie/SaaS", country = "Sénégal", onDataEnriched }: SearchAssistantProps) {
  ...
  const performSearch = async () => {
    ...
    const demoData = {
      marketData: { size: '850M FCFA', growth: '28%', trends: ['Cloud adoption', 'IA générative', 'API économie'], sector, country },
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 right-6 z-50 bg-gradient-to-r from-primary to-secondary text-white p-4 rounded-full shadow-lg hover:scale-105 transition-all"
      >
        <Search size={24} />
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden">
            ...
            <div className="flex overflow-x-auto border-b">
              {tabs.map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-4 py-2 text-sm whitespace-nowrap ${activeTab === tab.id ? 'border-b-2 border-primary text-primary' : 'text-gray-500'}`}>
            ...
            <div className="p-4 overflow-y-auto max-h-[60vh]">
              {isSearching ? (
                <div className="text-center py-8"><Loader2 className="animate-spin text-primary mx-auto" size={32} /><p className="mt-2">Recherche en cours...</p></div>
              ) : data ? (
                <div className="space-y-3">
                  ...
                  {activeTab === 'competitors' && data.competitors?.map((c: any, i: number) => <div key={i} className="border rounded p-2"><p className="font-bold">{c.name}</p><p>{c.strengths?.join(', ')}</p></div>)}
                  ...
                  {activeTab === 'news' && data.news?.map((n: any, i: number) => <div key={i} className="border-b py-2"><p className="font-semibold">{n.title}</p><p className="text-sm text-gray-500">{n.source}</p></div>)}
                  {activeTab === 'regulations' && <p>RGPD local, Loi FinTech, Zone OHADA</p>}
                </div>
              ) : <div className="text-center py-8 text-gray-500">Recherche de données...</div>}
            </div>
          </div>
        </div>
      )}
